...
nom = ["Martin", "Bernard", "Thomas", "Petit", "Robert", "Richard", "Durand",
...
def create_players(players_quantity):
    ...
    for i in range(players_quantity):
        ...
        manager_player.add_one(player, player_id)
    return players_obj
...
for index in range(4):
    # Création du tournoi
    tournament = Tournament(f"Tournoi privé {index + 1}",
                            "Le Havre",
                            f"0{index + 1}/11/2022",
                            "rapid",
                            f"Mon faux tournoi d'échecs {index + 1}")
...
{tournament.get_points(player_2.get_id)}")
    ...
    # Création des 3 rounds suivants
...
{player_1[0]} {player_2[0]}")
        ...
        # Création de la listes des instances de Match pour le round_x
        round_x_matches_list: List[Match] = []
        print()
